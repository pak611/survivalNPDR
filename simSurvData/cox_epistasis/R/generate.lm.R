...
#' simdata$beta
generate.lm <- function(baseline, X=NULL, N=1000, type="none", beta=NULL, xvars=3, mu=3, sd=1, censor=.1, interactions=FALSE, inter.mat=NULL,
     ...
     if(type=="none"){
          ...
          if(data.type=="genotype"){
               if(is.null(X)) X <- cbind(matrix(rbinom(N*xvars, 2, .5), N, xvars))

               # create  a 3D-matrix X.one.hot that is based on X only in one-hot encoding based on the genotype
               # One-hot encode the matrix
               X_encoded <- as.data.frame(X)
               
               # One-hot encode the genotype matrix
               X_encoded <- lapply(X_encoded, one.hot.encode)
               

               # generate the beta matrix which is equivalent in dimension to the one-hot encoded matrix
               if(is.null(beta)) {
                    # Create a list of matrices with the same dimensions as one-hot encoded matrices, filled with random normal values
                    beta <- lapply(X_encoded, function(matrix) {
                    nrow <- nrow(matrix)
                    ncol <- ncol(matrix)
                    matrix(rnorm(nrow * ncol, mean=0, sd=.1), nrow = nrow, ncol = ncol)})
               }

               # Multiply the one-hot encoded matrix with the beta matrix
               elementwise.product <- Map('*', X_encoded, beta)

               # sum the columns of each matrix to get the cumulative risk for each instance
               cumulative.risk <- lapply(elementwise.product, rowSums)

               # Combine the results into a dataframe
               cumulative.risk.df <- as.data.frame(cumulative.risk)


               
          } else { # data.type == "gene-expression"
          ...
          #print(head(X))
               #        [,1]       [,2]        [,3]        [,4]        [,5]       [,6]
               # [1,]  0.63852185 -0.1532569 -0.26353978 -0.07573792  0.44770266  0.5969117
               # [2,] -0.10345061 -0.1129649 -0.05246701  0.35915004  0.23611920 -0.7776085
               # [3,] -0.02780091 -0.7477000 -0.10746772 -0.04921657 -0.04299717  0.3861831

          if (interactions==TRUE){
               calc.interactions <- function(inst.vals, inter.mat){
                    #print("pairwise multiplicative interactions")
                    #browser()
                    prods.mat <- outer(inst.vals, inst.vals, '*')
                    weighted.prods <- inter.mat * prods.mat
                    sum.prods <- sum(weighted.prods)
                    #logistic.sum <- 1/(1+exp(-sum.prods))

                    return(sum.prods)
               }
               #browser()
               sum.prods <- sapply(1:nrow(X), function(i) calc.interactions(X[i,], inter.mat))
               XB <- XB + sum.prods
          }
          #browser()
...
#'
#' This function takes a data frame of genotype data where each entry is an integer representing a genotype 
#' and converts it into a one-hot encoded format. Each unique genotype is represented as a separate binary column.
#'
#' @param df A data frame where each column represents a gene and each entry is an integer genotype.
#'
#' @return A data frame with one-hot encoded columns for each unique genotype.
#'
#' @examples
#' # Simulate genotype data
#' set.seed(123)
#' X <- data.frame(Gene1 = sample(0:2, 100, replace = TRUE), 
#'                 Gene2 = sample(0:2, 100, replace = TRUE), 
#'                 Gene3 = sample(0:2, 100, replace = TRUE))
#'
#' # One-hot encode the genotype data
#' X_one_hot <- one.hot.encode(X)
#' print(head(X_one_hot))
#'
#' @export
one.hot.encode <- function(col) {
     classes <- sort(unique(col))
     n.classes <- length(classes)
     n.instances <- length(col)

     one.hot.matrix <- matrix(0, nrow = n.instances, ncol = n.classes)

     for (i in 1:n.instances) {
          class.index <- which(classes == col[i])
          one.hot.matrix[i, class.index] <- 1
     }
