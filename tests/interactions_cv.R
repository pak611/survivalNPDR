...
seeds <- c(1234, 5678, 91011, 121314, 151617, 181920, 212223, 242526, 272829, 303132)
...
for (iter in 1:num_iterations) {
    ...
    # Simulate data
    simdata <- simul.int(seeds[iter], n = 200, p = 500,
                         n.main = 2,
                         n.int = 50,
                         beta.main = 0.1, 
                         beta.int = 1.00, 
    ...
    pr_curve_survNPDR <- PRROC::pr.curve(scores.class0 = func_betas, 
                                         ...
                                         curve = TRUE)
    auc_results$npdr[iter] <- pr_curve_survNPDR$auc.integral
    ...
    func_betas_glmnet <- survNPDR_glmnet.model$beta[survNPDR_glmnet.model$Feature %in% interaction_vars]
    neg_betas_glmnet <- survNPDR_glmnet.model$beta[!survNPDR_glmnet.model$Feature %in% interaction_vars]
    pr_curve_npdr_glmnet <- PRROC::pr.curve(scores.class0 = abs(func_betas_glmnet), scores.class1 = abs(neg_betas_glmnet), curve = TRUE)
    auc_results$npdr_glmnet[iter] <- pr_curve_npdr_glmnet$auc.integral
    ...
    pr_curve_standard_cox <- PRROC::pr.curve(scores.class0 = abs(func_betas),
                                             ...
                                             curve = TRUE)
    auc_results$standard_cox[iter] <- pr_curve_standard_cox$auc.integral
    ...
    pr_curve_svm <- PRROC::pr.curve(scores.class0 = abs(func_betas), 
                                    scores.class1 = abs(non_func_betas), 
                                    curve = TRUE)
    auc_results$survival_svm[iter] <- pr_curve_svm$auc.integral
}
...
saveRDS(auc_results, file = "auc_results_interactions.rds")