...
gset <- getGEO("GSE32062", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]]
...
#pheno_data_file <- "C:/Users/patri/OneDrive/Desktop/snpdr_update2/snpdr_update/data/GSE_9891/GSE9891_clinical_anns.txt"
pheno_data_file <- "C:/Users/patri/OneDrive/Desktop/snpdr_update2/snpdr_update/data/GSE_32062/GSE32062_clinical_info.txt"
pheno_data <- read.csv(pheno_data_file, header = TRUE, stringsAsFactors = FALSE)

file_path <- "C:/Users/patri/OneDrive/Desktop/snpdr_update2/snpdr_update/data/GSE_32062/ID_REF_to_GSM.txt"
...
# If the data has more or fewer columns than expected, adjust the column names accordingly
# Assuming the data has two columns based on the provided example
colnames(data) <- c("GSM_NUM", "ID_REF_HG")

# Separate the ID_REF_HG column into ID_REF and HG_NUM
data <- data %>% separate(ID_REF_HG, into = c("ID_REF", "HG_NUM"), sep = " ")

# Print the first few rows of the data to verify the separation
print(head(data))
...
#pheno_data <- pheno_data[match(sample_names, pheno_data$geo_accn), ]
pheno_data <- pheno_data[match(sample_names, pheno_data$ID_REF), ]

# Check if the sample names match and handle missing values
if (any(is.na(pheno_data$ID_REF)) || !all(sample_names == pheno_data$ID_REF, na.rm = FALSE)) {
...
rfs_time <- pheno_data$time.rfs
rfs_status <- pheno_data$event.rfs
...
saveRDS(expr_data_final, file = "C:/Users/patri/OneDrive/Desktop/snpdr_update2/snpdr_update/data/GSE_32062/dat.rds")
