...
for (fold_idx in seq_along(k_folds)) {
  ...
  # DNNSurv (Deep Neural Network Survival)
  dnn_runtime <- system.time({
    dnn_fit <- tryCatch({
      DNNSurv(
        data.train = data.frame(time = train_data$time, status = train_data$status, attr_train),
        y = data.frame(time = train_data$time, status = train_data$status),
        data.test = data.frame(time = test_data$time, status = test_data$status, attr_test),
        y.test = data.frame(time = test_data$time, status = test_data$status),
        nodesize = 15,
        activation = "relu",
        batch_size = 32,
        epochs = 100,
  ...
  if (!is.null(dnn_fit) && !is.null(dnn_fit$predict.risk)) {
    # DNNSurv returns risk scores for the test set
    risk_scores <- dnn_fit$predict.risk
    c.ind.dnn <- concordance.index(x = risk_scores, surv.time = test_data$time, surv.event = test_data$status)
    errors <- rbind(errors, data.frame(method = "DNNSurv", fold = fold_idx, c_index = c.ind.dnn$c.index,
                                       auc = NA, runtime = dnn_runtime,
                                       n = n, p = p, n_main = n_main, n_int = n_int, beta_main = beta_main, beta_int = beta_int, censparam = censparam, lambda = lambda))
  }
