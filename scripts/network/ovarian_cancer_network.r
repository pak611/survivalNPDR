...
# Filter to keep the 500 genes with the highest variance
variance_values <- apply(attr_mat, 2, var) # Calculate variance for each column
high_variance_genes <- order(variance_values, decreasing = TRUE)[1:500]
filtered_attr_mat <- attr_mat[, high_variance_genes]
