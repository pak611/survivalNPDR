#!/usr/bin/env Rscript
# fetch_preprocess_breastcancer.R
#
...
# Paths
# ---------------------------------------------------------------------------
root      <- tryCatch(rprojroot::find_root(rprojroot::is_rstudio_project),
                      error = function(e) getwd())
data_root <- file.path(root, "survival_NPDR", "data")
...
if (file.exists(pheno_file2034)) {
  ...
  geo_col    <- grep("geo.acc|accession", names(pheno2034), ignore.case = TRUE, value = TRUE)[1]
