...
for (ds in datasets) {
  ...
  feature_data <- Biobase::fData(gset)
  ...
  expr_data_with_genes$gene <- as.character(expr_data_with_genes$gene)
  # For duplicate probes mapping to the same gene, retain the probe with highest mean expression
  sample_cols <- setdiff(names(expr_data_with_genes), c("ID", "gene"))
  expr_data_with_genes$row_mean <- rowMeans(expr_data_with_genes[, sample_cols], na.rm = TRUE)
  expr_data_with_genes <- expr_data_with_genes[order(expr_data_with_genes$gene,
                                                      -expr_data_with_genes$row_mean), ]
  expr_data_with_genes <- expr_data_with_genes[!duplicated(expr_data_with_genes$gene), ]
  expr_data_with_genes$row_mean <- NULL
